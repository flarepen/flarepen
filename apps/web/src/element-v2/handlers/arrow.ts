import { Element, ElementHandler, ArrowData } from '../types';
...
export const ArrowHandler: ElementHandler<ArrowData> = {
  type: 'arrow',
  ...
  create(x, y) {
    ...
    return {
      ...
      x, y,
      rows: g.polyarrow(points, false, true),
      data: { type: 'arrow', points, startArrow: false, endArrow: true },
      labelEnabled: false,
    };
  },
  ...
  preview(element, mouseMove) {
    ...
    return {
      ...
      x: minX,
      y: minY,
      rows: g.polyarrow(newPoints, element.data.startArrow, element.data.endArrow),
      data: { ...element.data, points: newPoints },
    };
  },
